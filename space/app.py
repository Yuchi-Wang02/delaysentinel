...
the model confidently answer anyway.
"""
...
FALLBACK_ID = os.environ.get("MODEL_ID_FALLBACK", "Yuchiwang02/DelaySentinel")
...
def predict_single(*values):
    ...
    swapped = fields.copy()
    swapped["Shipment_Status"] = "Delayed" if fields["Shipment_Status"] != "Delayed" else "In Transit"
    ...
    summary = (
        ...
        f"**Same row with `Shipment_Status` flipped to `{swapped['Shipment_Status']}`:** `{raw_swapped}`\n\n"
...
def predict_csv(file, drop_rule):
    ...
    msg = (
        f"{len(out)} rows scored in {dt:.1f} s. Model agrees with the rule on {agree_rate:.0%} of rows; "
        f"model positive rate {pos:.0%}."
    )
    if "Logistics_Delay" in frame.columns:
        gold = frame["Logistics_Delay"].astype(int)
        acc = sum(1 for lab, g in zip(labels, gold, strict=False) if lab == g) / len(out)
        msg += f" Accuracy against the file's own label column: {acc:.1%}."
    if drop_rule:
...
else and it does not; the second and third lines of the result show the same row with the
status flipped and with both rule fields deleted. Model card, evaluation JSON and probes:
...
"""
    ...
    with gr.Tab("CSV (up to 200 rows)"):
        gr.Markdown(
            "Upload the Kaggle CSV or any file with the same 15 columns. Optionally strip the two rule fields from every prompt first."  # noqa: E501
        )
    ...
    gr.Markdown(
        """
<small>Weights: Llama 3.2 Community License (see LICENSE and USE_POLICY.md in the model repo). Data: Kaggle ziya07, CC0.
Every number in the model card comes from <code>results/eval.json</code> in the GitHub repository. This demo runs the
published bf16 weights on CPU; the first request loads the model and may take a minute.</small>
"""
    )
