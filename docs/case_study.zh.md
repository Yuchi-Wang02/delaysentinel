...
问它"法国的首都是哪里"，它回答 `Paris`。底模还在里面；微调加上的是一个字符串匹配器，外加一个"表单在但字符串
不在时就答 0"的强先验。

仍然没有答案的是：到底匹配的是什么模式。字段名 `Logistics_Delay_Reason` 在每一条 prompt 里都含有 "Delay"
却不触发任何东西，所以不是朴素的子串扫描；`Late`、`Early`、`Light` 为什么触发也没有测过。这些是我接下来会
设计的探针，而它们本该在训练前就写进协议，而不是一年以后。
