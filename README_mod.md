...
Every number on this page is produced by `python -m delaysentinel.eval` and stored in
[`results/eval.json`](results/eval.json); CI fails if the README quotes a number that file does not
contain (`scripts/check_card_numbers.py`). Code, tests and figures:
...
- **What the model learned.** Editing the rule fields flips 200 of 200 predictions in the expected
...
**Split.** 200 test rows (116 positive / 84 negative, positive rate 0.75) from an **unseeded**
