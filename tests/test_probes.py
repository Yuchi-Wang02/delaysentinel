...
from delaysentinel.probes import _literal_rule_on_texts, counterfactual_probes, ood_probes, robustness_probes
...
def test_counterfactual_probe_sizes(test_frame, test_rows):
    ...
    for key, fields in (
        ...
        for i, t in zip(p.indices, p.texts, strict=False):
            ...
            assert 1 <= len(diff) <= len(fields) and set(diff) <= fields
...
def test_robustness_probe_construction(test_frame, test_rows):
    ...
    assert len(probes["synonym_Delayed_to_Late"].texts) == 50 + 23  # every Delayed row in the test split
    assert all("Shipment_Status: Late" in t for t in probes["synonym_Delayed_to_Late"].texts)
